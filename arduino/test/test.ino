/* Last modified: <25-Nov-2015 15:58:59 CET by Dmitry Ebel> */
...
void setup() { 
    ...
    sample_t in[BUFFER_LEN];
    for (int16_t i=0; i<BUFFER_LEN; i++) {
	pd_put_sample(&pd, i);
    }
    //    pd_pack_buffer(&pd, &in, BUFFER_LEN);
    
    msg.header = MSG_START;
    msg.type   = MSG_TYPE_CMP;
    msg.data_size = pd.buffer_len_bytes;
	...
	SerialUSB.write((byte*)&msg, sizeof(msg));
	for (i=0; i<msg.data_size/CHUNK_SIZE; i++) {
	    SerialUSB.write(pd.data+i*CHUNK_SIZE, CHUNK_SIZE);
	}
	SerialUSB.write(pd.data+i*CHUNK_SIZE, msg.data_size%CHUNK_SIZE );
    }

/* Last modified: <27-Nov-2015 11:59:36 CET by Dmitry Ebel> */
...
static packed_data_t pd;

inline void set_acq_flag1(void)   {msg.timestamp = micros(); acq_flag = 1; beacon_id = 0;}
inline void set_acq_flag2(void)   {msg.timestamp = micros(); acq_flag = 1; beacon_id = 1;}
inline void set_acq_flag3(void)   {msg.timestamp = micros(); acq_flag = 1; beacon_id = 2;}
inline void set_acq_flag4(void)   {msg.timestamp = micros(); acq_flag = 1; beacon_id = 3;}
inline void set_acq_flag5(void)   {msg.timestamp = micros(); acq_flag = 1; beacon_id = 4;}
inline void set_acq_flag6(void)   {msg.timestamp = micros(); acq_flag = 1; beacon_id = 5;}
...
void ADC_Handler (void) {
    ...
    if (acq_flag) {
	digitalWriteDirect(LED_RED, HIGH);
	...
	if (writeloc == BUFFER_LEN) {
	    ...
	    digitalWriteDirect(LED_RED, LOW);
	}
    }
}
...
#define CHUNK_SIZE 512 /*bytes*/
    ...
    pd_pack_buffer(&pd, &rawsig7[0], BUFFER_LEN);
    data = (byte*)(pd.data);
    for (i=0; i<msg.data_size/CHUNK_SIZE; i++) {
	SerialUSB.write(data+CHUNK_SIZE*i, CHUNK_SIZE);
    }
    SerialUSB.write(data+CHUNK_SIZE*i, msg.data_size%CHUNK_SIZE);
    
    msg.sensor_id = 1;
    SerialUSB.write((byte*)&msg, sizeof(msg));
    pd_pack_buffer(&pd, &rawsig6[0], BUFFER_LEN);
    data = (byte*)(pd.data);
        for (i=0; i<msg.data_size/CHUNK_SIZE; i++) {
	SerialUSB.write(data+CHUNK_SIZE*i, CHUNK_SIZE);
    }
    ...
    msg.sensor_id = 2;
    SerialUSB.write((byte*)&msg, sizeof(msg));
    pd_pack_buffer(&pd, &rawsig5[0], BUFFER_LEN);
    data = (byte*)(pd.data);
    for (i=0; i<msg.data_size/CHUNK_SIZE; i++) {
	SerialUSB.write(data+CHUNK_SIZE*i, CHUNK_SIZE);
    }
    ...
    msg.sensor_id = 3;
    SerialUSB.write((byte*)&msg, sizeof(msg));
    pd_pack_buffer(&pd, &rawsig4[0], BUFFER_LEN);
    data = (byte*)(pd.data);
    for (i=0; i<msg.data_size/CHUNK_SIZE; i++) {
	SerialUSB.write(data+CHUNK_SIZE*i, CHUNK_SIZE);
    }
    ...
    msg.sensor_id = 4;
    SerialUSB.write((byte*)&msg, sizeof(msg));
    pd_pack_buffer(&pd, &rawsig3[0], BUFFER_LEN);
    data = (byte*)(pd.data);
    for (i=0; i<msg.data_size/CHUNK_SIZE; i++) {
	SerialUSB.write(data+CHUNK_SIZE*i, CHUNK_SIZE);
    }
    ...
    msg.sensor_id = 5;
    SerialUSB.write((byte*)&msg, sizeof(msg));
    pd_pack_buffer(&pd, &rawsig2[0], BUFFER_LEN);
    data = (byte*)(pd.data);
    for (i=0; i<msg.data_size/CHUNK_SIZE; i++) {
	SerialUSB.write(data+CHUNK_SIZE*i, CHUNK_SIZE);
    }
...
void setup() { 
    ...
    while (!SerialUSB) ;
    REPORT("calling set_adc_frequency()");
    set_sample_rate(REC_SAMPLE_RATE);
    REPORT("calling adc0_setup()");
    adc0_setup();

    REPORT("Setting pin modes");
    ...
    REPORT("Setting extern interrupts");
    ...
    msg.header = MSG_START;
    msg.type   = MSG_TYPE_CMP;
    msg.data_size = pd.buffer_len_bytes;//BUFFER_LEN*sizeof(sample_t);

    digitalWriteDirect(13, LOW);
    REPORT("Leaving setup()");
